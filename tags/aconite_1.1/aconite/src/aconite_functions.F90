...
!-----------------------------------------------------------------------
          ...
          if(abs(C_flux - delta_stateC) > 0.1) then
...
!-----------------------------------------------------------------------
      ...
      if(new_year ==  1) then
          ...
          flux%annual_leachN = flux%annual_leachN + flux%leachN
          flux%annual_leachDON = flux%annual_leachDON + flux%leachDON
...
! !IROUTINE: integrated marginal returns
  subroutine marginal_integrator()
   ...
   implicit none 
    ...
    real :: leaf_horizon,root_horizon, leafc_adjust, tmp, marg_ineff
    real :: rootc_adjust,tmpmaxleafC,leafN_deficit, leafn_adjust, delta_leafCN
    real :: delta_maxleafC,delta_maxleafN,delta_maxrootC,no_n_adjust
!EOP
!-----------------------------------------------------------------------
   ...
   if(clim%doy(state%rstep) == 365) then  !ANNUAL UPDATE
       ...
       root_horizon = (param%t_root*365.)
 	   ...
 	   marg%integ_Creturn_leafC =((marg%integ_GPP_leafC - marg%integ_Rm_leafC) &
 			/leaf_horizon - marg%integ_Rg_leafC - param%add_C)
       ...
       !CALCULATE LEAF C ALLOCATION   
   
       if(marg%integ_hitmaxleafC > 0.99 .and. (marg%integ_Creturn_leafC) >= 0.0) then
           if(state%min_wood_deficit == 0.0 .and. marg%integ_hitmaxrootC > 0.99 ) then
               delta_maxleafC = state%maxleafC *  (1+leafc_adjust)
               state%debug = 1
           else
           	   !leafc_adjust=min(leafc_adjust,max(1-marg%integ_hitmaxrootC,marg%integ_hitwood_requirement))
               delta_maxleafC = state%maxleafC*(1-leafc_adjust)
               state%debug = 2
           endif
       endif
       ...
       !param%leafN_deficit_thres      
       if(marg%integ_hitmaxleafC <= 0.99  .and. (leafN_deficit > param%leafN_deficit_thres  &
       .or. marg%integ_Creturn_leafC < 0.0)) then
            delta_maxleafC = state%maxleafC*(1-leafc_adjust)     
            state%debug = 4 
       elseif(marg%integ_hitmaxleafC <= 0.99  .and. (leafN_deficit <= param%leafN_deficit_thres &
       .or. marg%integ_Creturn_leafC >= 0.0)) then
       		delta_maxleafC = state%maxleafC
       		state%debug = 5
       endif
       
       !CALCULATE LEAF N ALLOCATION 
       
       leafn_adjust =leafn_adjust
       
       if(state%leafC > 0) then
           state%maxleafN = state%maxleafC*(state%leafN/state%leafC)
       endif

       if(marg%integ_hitmaxleafC > 0.99 .and. leafN_deficit > param%leafN_deficit_thres &
       .and. marg%integ_Creturn_leafN > 0.0) then
       		if(state%min_wood_deficit == 0.0 .and. marg%integ_hitmaxrootC > 0.99) then
       			 delta_maxleafN = state%maxleafN*(1+leafn_adjust)   
       			 !state%debug2 = 1
       		else
                delta_maxleafN = state%maxleafN   !we may not need this
                !state%debug2 = 2
            endif  
       endif
       	
       	if(marg%integ_hitmaxleafC > 0.99 .and. leafN_deficit <= param%leafN_deficit_thres) then
       	     delta_maxleafN = state%maxleafN*(1-leafn_adjust)
       	     !state%debug2 = 3
       	endif
...
! CALCULATE ROOT C AND N ALLOCATION

       marg_ineff = (marg%integ_Nreturn_rootC-marg%integ_Nreturn_Raexcess)
       rootc_adjust = min(param%MaxallocAdjust,(param%MaxallocAdjust*marg_ineff))

	   if(marg%integ_hitmaxrootC > 1.00) then
			delta_maxrootC = state%maxrootC * marg%integ_hitmaxrootC
	   endif
			
       if(marg%integ_hitmaxrootC > 0.99 &
       .and. marg%integ_Nreturn_rootC > marg%integ_Nreturn_Raexcess &
       .and.  marg%integ_Nreturn_rootCN > marg%integ_addN_rootN .and.( &
        leafN_deficit <= param%leafN_deficit_thres .or. &
        marg%integ_Creturn_leafN >marg%integ_Creturn_leafC)) then
            delta_maxrootC = state%maxrootC * (1+rootc_adjust) 
            !state%debug2 = 1				  
       endif
             
       if(marg%integ_hitmaxrootC  > 0.99 .and.( &
       marg%integ_Nreturn_rootC <= marg%integ_Nreturn_Raexcess .or. &
       marg%integ_Nreturn_rootCN <= marg%integ_addN_rootN)) then            
            delta_maxrootC= state%maxrootC * (1-rootc_adjust)   
            !state%debug2 = 2    
       endif
             
       if(marg%integ_hitmaxrootC < 0.99) then   
       		rootc_adjust = min(rootc_adjust, 1-marg%integ_hitmaxrootC)
            delta_maxrootC = state%maxrootC * (1-rootc_adjust)
			!state%debug2 = 3
       endif
       
       if(marg%integ_hitmaxrootC > 0.99 .and. leafN_deficit > param%leafN_deficit_thres) then
       	  delta_maxrootC  = state%maxrootC * (1-rootc_adjust)
	   	  !state%debug2 = 4	     	
       endif
        
        delta_maxrootC = max((state%maxleafC * param%leaf2root_ratio),state%maxrootC)
        
		state%maxleafC = state%maxleafC - (state%maxleafC - delta_maxleafC)*min(1.,(param%damp_rate/leaf_horizon))
		...
		!state%target_leafCN = 45
		!state%maxleafN = state%maxleafC/45
		
		!update leaf traits based on target leaf CN, using Reich PNAS slopes
        !param%lca = 10^( int + slope*log10(state%target_leafCN))
        !param%t_leaf = state%target_leafCN
...
!-----------------------------------------------------------------------
      ...
      flux%rootN_dist_litter =0.0
      flux%labileC_dist_atm =0.0
      flux%labileC_dist_litter =0.0	
      flux%labileC_bud_dist_atm =0.0
      flux%labileC_bud_dist_litter =0.0
      flux%labileC_Ra_dist_atm =0.0
      flux%labileC_Ra_dist_litter =0.0	
      flux%labileN_dist_atm =0.0
	  ...
	  flux%dist_cwdC =0.0
	  flux%dist_cwdN  =0.0
	  flux%dist_litterC_to_soil1 =0.0
...
!LEAF N
    ...
    marg%GPP_leafN = Photosyn((state%leafN + marg%addN_leafN), state%lai, state%rstep) - flux%GPP
    if(param%use_reich == 0) then
...
! LEAF C AND N 
    ...
    marg%GPP_leafCN = Photosyn((state%leafN + marg%addN_leafN), add_lai, state%rstep) - flux%GPP
...
!ROOTC	
    ...
    !Labile N
    marg%Nreturn_Raexcess = param%add_C * param%Nfix_per_gC*Ra_temp_resp*state%Nuptake_downreg
