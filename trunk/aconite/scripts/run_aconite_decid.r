...
run_aconite <- function(file1,file2,plot_day,total_sim,start,end,annual_doy,spinup_length){
        ...
        model_dir = '/Users/rqthomas/Dropbox/ACONITE/ACONITE_V1.1/'
        setwd(model_dir)
        
        input_dir = paste(model_dir,'input_data/',sep='')
        output_dir = paste(model_dir,'output/',sep='')

        CLIM_FILE=paste(input_dir,'climate_HF_2002_2012_noleap.csv',sep='')
        PARAM_FILE=paste(input_dir,'parameter_temp_decid.txt',sep='')
        if(file1!=''){
       		 DAY_OUT=paste(output_dir,file1,sep='')
       	}else{
       		 DAY_OUT=''
       	}
        ANNUAL_OUT=paste(output_dir,file2,sep='')
        RESTART_IN=paste(input_dir,'restart_temp_decid.txt',sep='')      
        RESTART_OUT=paste(input_dir,'restart_test.txt',sep='')
	    DIST_IN=''
	    SITE_IN=paste(input_dir,'site_details_HF.txt',sep='')
	    NFERT_IN=paste(input_dir,'nfert_HF.txt',sep='')
	    SOILWARM_IN=''
        ...
        setwd(paste(model_dir,'scripts',sep=''))
